...
vi.mock("@/auth", () => ({
  auth: vi
    .fn()
    .mockResolvedValue({ user: { id: "u-student", role: "STUDENT" } }),
}));
...
describe("GET /api/students/me/class-sessions (classTypeIds filter)", () => {
  ...
  it("applies exact-match IN filter when classTypeIds is provided", async () => {
    ...
    const url =
      "http://localhost/api/students/me/class-sessions?startDate=2025-01-01&endDate=2025-01-07&classTypeIds=a,b";
