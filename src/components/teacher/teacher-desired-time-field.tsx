...
interface TeacherDesiredTimeFieldProps {
  form: UseFormReturn<TeacherShiftPreferencesInput>
}
        ...
        <FormItem>
          ...
          <div className="space-y-4">
                ...
                >
                  ...
                  <SelectContent>
                    {Object.entries(dayOfWeekMap)
                      .filter(([, displayValue]) => !(field.value || []).some((time) => time.dayOfWeek.toUpperCase() === displayValue.toUpperCase()))
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  const weekdayExists = (field.value || []).some(
                    (time) => time.dayOfWeek.toUpperCase() === selectedWeekday.toUpperCase()
                  );
                  if (weekdayExists) {
                    alert("この曜日のシフト希望時間は既に追加されています");
                    return;
                  }
            ...
            <div className="flex flex-wrap gap-2 mt-2">
              {(field.value || []).map((time, index: number) => {
