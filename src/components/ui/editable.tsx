...
interface EditableRootProps extends Omit<React.HTMLAttributes<HTMLDivElement>, 'onSubmit'> {
  asChild?: boolean
  invalid?: boolean
  required?: boolean
  readOnly?: boolean
...
const EditableRoot = React.forwardRef<HTMLDivElement, EditableRootProps>(
  (
    ...
    }, [editing, value, defaultValue, onEscapeKeyDown, onEnterKeyDown, onSubmit, onCancel, setEditing, setValue])
    ...
    const compProps = asChild ? {} : { className: cn("relative", className), ref, ...props }
...
const EditableArea = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & { asChild?: boolean }>(
...
const EditablePreview = React.forwardRef<HTMLDivElement, React.HTMLAttributes<HTMLDivElement> & { asChild?: boolean }>(
  ({ className, asChild = false, children, onClick, ...props }, ref) => {
    const { value, disabled, readOnly, editing, placeholder, setEditing, triggerMode } = useEditableContext()
...
>(({ className, asChild = false, onBlur, ...props }, ref) => {
  const { value, setValue, editing, placeholder, name, id, disabled, readOnly, autosize, onSubmit, setEditing } =
    useEditableContext()
  ...
  return (
    <Input
      ref={inputRef}
      className={cn("w-full", autosize && "resize-none overflow-hidden", className)}
      value={value}
...
>(({ className, asChild = false, forceMount = false, onClick, ...props }, ref) => {
  const { setEditing, editing, disabled, readOnly, triggerMode } = useEditableContext()
...
>(({ className, asChild = false, orientation = "horizontal", children, ...props }, ref) => {
  ...
  const compProps = asChild
    ? {}
    : {
        className: cn("flex gap-2 mt-2", orientation === "vertical" ? "flex-col" : "flex-row", className),
        ref,
        ...props,
      }
...
>(({ className, asChild = false, onClick, ...props }, ref) => {
  ...
  return <Button ref={ref} className={cn(className)} onClick={handleClick} {...props} />
})
