...
export const AdminCalendarList = () => {
  ...
  useEffect(() => {
    console.log(
      "API Base URL:",
      process.env.NEXT_PUBLIC_API_BASE_URL || "(empty)"
    );
  }, []);
      ...
      try {
        ...
        if (!response.ok) {
          ...
          try {
            ...
            console.error(
              "Session check failed but couldn't parse response:",
              parseError
            );
          }
          return false;
        ...
        try {
          ...
          console.error(
            "Session exists but couldn't parse response:",
            parseError
          );
  ...
  const filteredSessions = useMemo(() => {
    return allSessions.filter((session) => {
      if (selectedTeacherId && session.teacherId !== selectedTeacherId)
        return false;
      if (selectedStudentId && session.studentId !== selectedStudentId)
        return false;
  ...
  const deleteSessionMutation = useMutation({
        ...
        try {
          ...
          if (!response.ok) {
            ...
            try {
              ...
              console.error(
                "Delete failed but couldn't parse response:",
                parseError
              );
            }
    ...
    onError: (error) => {
      toast.error(
        error instanceof Error ? error.message : "授業の削除に失敗しました"
      );
    },
  });
  ...
  const handleEdit = async (session: ClassSession) => {
    try {
      ...
      if (exists) {
        console.log(
          "Session exists, opening LessonDialog for session ID:",
          session.classId
        );
  ...
  const sortedSessions = [...paginatedSessions].sort((a, b) => {
    ...
    if (dateComparison !== 0)
      return timeSort === "asc" ? dateComparison : -dateComparison;
    ...
    if (hourComparison !== 0)
      return timeSort === "asc" ? hourComparison : -hourComparison;
                      ...
                      if (teacher.compatibilityType === "perfect") {
                        ...
                        if (teacher.partialMatchingSubjectsCount > 0) {
                          ...
                          partialMatchingSubjectsCount =
                            teacher.partialMatchingSubjectsCount;
                        }
                        ...
                        teacher.compatibilityType === "teacher-no-prefs"
                      ) {
                        description = "科目設定なし";
                      } else if (
                        teacher.compatibilityType === "student-no-prefs"
                      ) {
                    ...
                    .map((student) => {
                      ...
                      if (student.compatibilityType === "perfect") {
                        ...
                        if (student.partialMatchingSubjectsCount > 0) {
                          ...
                          partialMatchingSubjectsCount =
                            student.partialMatchingSubjectsCount;
                        }
                        ...
                        student.compatibilityType === "student-no-prefs"
                      ) {
                        description = "科目設定なし";
                      } else if (
                        student.compatibilityType === "teacher-no-prefs"
                      ) {
              ...
              <TableBody>
                {sortedSessions && sortedSessions.length > 0 ? (
                  sortedSessions.map((session) => {
                    const isConflicted =
                      (session as any)?.status === "CONFLICTED";
                    const stripePx = 3;
                    ...
                    return (
                      ...
                      >
                        <TableCell
                          className={
                            isConflicted
                              ? "border-l-4 border-destructive/80"
                              : ""
                          }
                          style={rowStyle}
                        >
