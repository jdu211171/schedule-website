...
const DayCalendarComponent: React.FC<DayCalendarProps> = ({
  ...
  const onDragStart = useCallback((e: DragStartEvent) => {
    ...
    // Pre-compute duration slots for ghost sizing
    ...
    // Mirror the card's slot-based width by using the indices from timeSlots
  ...
  const onDragOver = useCallback((e: DragOverEvent) => {
    ...
    // Clamp start index so the ghost fits within the row
  ...
  const onDragEnd = useCallback((e: DragEndEvent) => {
    ...
    // droppable id = cell-{boothIndex}-{timeIndex}
    const parts = overId.split('-');
    ...
    // If part of a series, let the user choose scope (single vs all)
    if (lesson.seriesId) {
      ...
      >
        ...
        >
          ...
          >
            ...
            {/* Ghost overlay (no pointer events) */}
            <div
              ...
              {ghost && (
                <div
                  ...
                  style={{
                    ...
                    top: `${ghost.boothIdx * TIME_SLOT_HEIGHT}px`,
                    width: `${ghost.durationSlots * CELL_WIDTH}px`,
                    height: `${TIME_SLOT_HEIGHT - 2}px`,
                  }}
                />
              )}
            </div>

            {/* Lessons overlay (container ignores pointers; cards handle their own) */}
            <div
              className="absolute pointer-events-none"
              style={{
                zIndex: 11,
          ...
          {/* Scope selection dialog for series updates */}
              ...
              <DialogFooter className="gap-2 sm:gap-2">
                ...
                <Button
                  onClick={() => {
                    ...
                    // Optimistic: update all cached classSessions matching series from this instance's date forward
