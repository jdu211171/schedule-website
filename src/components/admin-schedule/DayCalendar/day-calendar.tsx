...
// Определение типа для позиции выбора
export type SelectionPosition = {
  row: number;
  col: number;
};

// Переименуем интерфейс Room в Booth
interface Booth {
  boothId: string;
  name: string;
}
...
type DayCalendarProps = {
  date: Date;
  booths: Booth[]; // Переименовано с rooms на booths
  timeSlots: TimeSlot[];
...
const CELL_WIDTH = 40; 
const BOOTH_LABEL_WIDTH = 100; // Переименовано с ROOM_LABEL_WIDTH
...
const isCellInSelection = (
  boothIndex: number, // Переименовано с roomIndex
  timeIndex: number, 
...
const CalendarCell = React.memo(({ 
  boothIndex, // Переименовано с roomIndex
  timeSlot, 
  isSelected,
  isSelecting,
  onMouseDown,
  ...
  const cellKey = `cell-${boothIndex}-${timeSlot.index}`; // Переименовано с roomIndex
  
  return (
    <div
      key={cellKey}
      id={cellKey}
      data-booth-index={boothIndex} // Переименовано с data-room-index
  ...
  return prevProps.boothIndex === nextProps.boothIndex && // Переименовано с roomIndex
...
// Переименовано RoomRow в BoothRow
const BoothRow = React.memo(({ 
  booth, // Переименовано с room
  boothIndex, // Переименовано с roomIndex
  timeSlots,
  selectionStart,
  selectionEnd,
  ...
  booth: Booth, // Переименовано с room
  boothIndex: number, // Переименовано с roomIndex
  timeSlots: TimeSlot[],
  selectionStart: SelectionPosition | null,
  selectionEnd: SelectionPosition | null,
  isSelecting: boolean,
  onStartSelection: (boothIndex: number, timeIndex: number, e: React.MouseEvent) => void, // Переименовано с roomIndex
  onCellHover: (boothIndex: number, timeIndex: number, e: React.MouseEvent) => void, // Переименовано с roomIndex
  ...
  return (
    ...
    >
      <div
        ...
        style={{ 
          width: `${BOOTH_LABEL_WIDTH}px`, // Переименовано с ROOM_LABEL_WIDTH
          minWidth: `${BOOTH_LABEL_WIDTH}px`, // Переименовано с ROOM_LABEL_WIDTH
          height: `${TIME_SLOT_HEIGHT}px`,
          zIndex: 5
        }}
      >
        <span className="truncate">{booth.name}</span> {/* Переименовано с room.name */}
      </div>

      {timeSlots.map((timeSlot) => {
        const isSelected = isCellInSelection(boothIndex, timeSlot.index, selectionStart, selectionEnd); // Переименовано с roomIndex
        
        return (
          <CalendarCell
            key={`cell-${boothIndex}-${timeSlot.index}`} // Переименовано с roomIndex
            boothIndex={boothIndex} // Переименовано с roomIndex
            timeSlot={timeSlot}
            isSelected={isSelected}
            isSelecting={isSelecting}
            onMouseDown={(e) => onStartSelection(boothIndex, timeSlot.index, e)} // Переименовано с roomIndex
            onMouseEnter={(e) => onCellHover(boothIndex, timeSlot.index, e)} // Переименовано с roomIndex
  ...
  return prevProps.boothIndex === nextProps.boothIndex && // Переименовано с roomIndex
         prevProps.booth.boothId === nextProps.booth.boothId && // Переименовано с room.boothId
         prevProps.isSelecting === nextProps.isSelecting &&
         prevProps.selectionStart === nextProps.selectionStart &&
         prevProps.selectionEnd === nextProps.selectionEnd;
});

BoothRow.displayName = 'BoothRow'; // Переименовано с 'RoomRow'
...
const DayCalendarComponent: React.FC<DayCalendarProps> = ({
  ...
  const handleStartSelection = useCallback((boothIndex: number, timeIndex: number, e: React.MouseEvent) => { // Переименовано с roomIndex
    e.preventDefault();
    
    const start = { row: boothIndex, col: timeIndex }; // Переименовано с roomIndex
  ...
  const handleCellHover = useCallback((boothIndex: number, timeIndex: number, e: React.MouseEvent) => { // Переименовано с roomIndex
    if (!selection.isSelecting || !selection.start) return;
    e.preventDefault();
    
    if (boothIndex === selection.start.row) { // Переименовано с roomIndex
      setSelection(prev => ({
        ...prev,
        end: { row: boothIndex, col: timeIndex } // Переименовано с roomIndex
      }));
    }
  ...
  const handleEndSelection = useCallback((e: React.MouseEvent) => {
    if (selection.isSelecting && selection.start && selection.end && !createLessonCalledRef.current) {
      ...
      const boothIndex = selection.start.row; // Переименовано с roomIndex
      
      const isValidSelection = endCol > startCol; 
      if (isValidSelection) {
        if (startCol >= 0 && startCol < timeSlots.length &&
            endCol >= 0 && endCol < timeSlots.length &&
            boothIndex >= 0 && boothIndex < booths.length) { // Переименовано с rooms.length

          const startTime = timeSlots[startCol].start;
          const endTime = timeSlots[endCol].end; 
          const selectedBoothId = booths[boothIndex].boothId; // Переименовано с rooms[roomIndex].boothId
  ...
  }, [selection, timeSlots, booths, onCreateLesson, date, cancelSelection]); // Переименовано с rooms
    ...
    const handleGlobalMouseUp = (event: MouseEvent) => {
      if (selection.isSelecting && !createLessonCalledRef.current) {
        if (containerRef.current && !containerRef.current.contains(event.target as Node)) {
          cancelSelection();
        } else if (!(event.target as HTMLElement)?.closest('[data-booth-index][data-time-index]')) { // Переименовано с data-room-index
          cancelSelection();
        }
      }
    };
