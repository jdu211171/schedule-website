...
import { useMultipleDaysClassSessions, ExtendedClassSessionWithRelations } from "@/hooks/useClassSessionQuery";
...
const TIME_SLOTS: { index: number; start: string; end: string; display: string; shortDisplay: string }[] = Array.from({ length: 57 }, (_el, i) => {
...
export default function FastDayCalendarDialog({ open, onOpenChange, date, onAfterChange }: Props) {
  const day = useMemo(() => (typeof date === 'string' ? new Date(`${date}T12:00:00`) : date), [date]);
  ...
  const sessions = (classSessionQueries[0]?.data?.data || []) as ExtendedClassSessionWithRelations[];
  const isFetching = Boolean(classSessionQueries[0]?.isFetching || classSessionQueries[0]?.isLoading);
  ...
  const onLessonClick = useCallback((lesson: ExtendedClassSessionWithRelations) => {
  ...
  const formattedDate = useMemo(() => format(day, 'yyyy年MM月dd日 (eee)', { locale: ja }), [day]);
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        {selectedLesson && (
          <LessonDialog
            open={!!selectedLesson}
            onOpenChange={(o) => { if (!o) setSelectedLesson(null); }}
