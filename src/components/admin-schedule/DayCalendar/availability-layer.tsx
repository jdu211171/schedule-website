...
function subtractSlots(
  base: { startTime: string; endTime: string }[],
  sub: { startTime: string; endTime: string }[]
) {
  ...
  for (const s of subtractors) {
    ...
    for (const b of remaining) {
      ...
      if (sStart <= bStart && sEnd < bEnd) {
        next.push({ startTime: fromMin(sEnd), endTime: fromMin(bEnd) });
        continue;
      }
      if (sStart > bStart && sEnd >= bEnd) {
        next.push({ startTime: fromMin(bStart), endTime: fromMin(sStart) });
        continue;
      }
      if (sStart > bStart && sEnd < bEnd) {
        next.push({ startTime: fromMin(bStart), endTime: fromMin(sStart) });
...
function mergeSlots(slots: { startTime: string; endTime: string }[]) {
  ...
  for (let i = 1; i < sorted.length; i++) {
    ...
    if (toMin(sorted[i].startTime) <= toMin(last.endTime)) {
      last.endTime = fromMin(
        Math.max(toMin(last.endTime), toMin(sorted[i].endTime))
      );
    } else {
