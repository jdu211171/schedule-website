...
export default function SeriesSessionsTableDialog({
  seriesId,
  open,
  onOpenChange,
  softWarningDates = [],
  ...
  const [lessonDialogMode, setLessonDialogMode] = useState<"view" | "edit">(
    "edit"
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const conf = await fetch("/api/scheduling-config?scope=branch", {
          headers: {
            "X-Selected-Branch": localStorage.getItem("selectedBranchId") || "",
          },
        });
      ...
      if (seriesId) {
        try {
          const pv = await fetch(
            `/api/class-series/${seriesId}/extend/preview?months=${generationMonths}`,
            {
              headers: {
                "X-Selected-Branch":
  ...
  const allSelected =
    items.length > 0 && items.every((i) => selected[i.classId]);
  ...
  const softSet = useMemo(
    () => new Set(softWarningDates || []),
    [softWarningDates]
  );
        ...
        selectedIds.map((id) =>
          fetch(`/api/class-sessions/${id}`, {
            ...
            if (!r.ok)
              return r
                .json()
                .then((j) => Promise.reject(new Error(j?.error || "")));
          })
        )
      );
          ...
          <div className="mb-2 flex items-center justify-between gap-2">
            <div className="flex items-center gap-3 text-sm text-muted-foreground">
              ...
              {selectedIds.length > 0 && (
                <div>選択: {selectedIds.length}件</div>
              )}
            </div>
                  ...
                  <TableHead className="w-9">
                    <div className="flex items-center justify-center">
                      <Checkbox
                        checked={
                          allSelected ||
                          ((someSelected && "indeterminate") as any)
                        }
                ...
                {loading ? (
                  ...
                  items.flatMap((s) => {
                    const conflictsForDate =
                      preview?.conflictsByDate?.[s.date] || [];
                    const hasConflicts =
                      conflictsForDate.length > 0 || s.status === "CONFLICTED";
                    const expanded = !!expandedDates[s.date];
                    const mainRow = (
                      <TableRow
                        key={s.classId}
                        className={hasConflicts ? "bg-destructive/5" : ""}
                      >
                                  ...
                                  if (next) {
                                    ...
                                    if (
                                      ...
                                      try {
                                        ...
                                        const r = await fetch(
                                          `/api/class-sessions/${s.classId}/conflicts`,
                                          {
                                            headers: {
                                              "X-Selected-Branch":
                            ...
                            <Button
                              ...
                              onClick={async () => {
                                ...
                                try {
                                  ...
                                  const j = await res.json();
                                  const lesson = (j?.data ||
                                    j) as ExtendedClassSessionWithRelations;
                      ...
                      <TableRow key={`${s.classId}-details`}>
                        ...
                        <TableCell colSpan={7} className="bg-muted/30 py-3">
                          {(() => {
                            ...
                            const list = conflictsForDate.length
                              ? conflictsForDate
                              : extra;
                            ...
                            return list.length ? (
                              <div className="flex flex-col gap-2">
                                {list.map((c, idx) => (
                                  ...
                                  >
                                    <span
                                      className={`px-1 py-0.5 rounded text-xs border ${conflictTypeColor(c.type)}`}
                                    >
