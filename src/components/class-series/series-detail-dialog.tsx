...
export default function SeriesDetailDialog({
  seriesId,
  open,
  onOpenChange,
}: Props) {
  ...
  const dowsLabel = useMemo(
    () =>
      (series?.daysOfWeek || []).map((d) => dowsJa[d] || String(d)).join("・"),
    [series?.daysOfWeek]
  );
  ...
  const isDirty = useMemo(() => {
    return (
      ...
      JSON.stringify(form.daysOfWeek) !==
        JSON.stringify(series?.daysOfWeek || [])
    );
  }, [
    form,
    series?.status,
  ...
  const save = async () => {
    ...
    if (form.startDate && form.startDate !== series.startDate)
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-3 gap-4">
              <div className="space-y-1">
                <Label>講師</Label>
                <Select
                  value={(form.teacherId ?? "none") as string}
                  onValueChange={(v) =>
                    setForm((f) => ({
                      ...f,
              ...
              <div className="space-y-1">
                <Label>生徒</Label>
                <Select
                  value={(form.studentId ?? "none") as string}
                  onValueChange={(v) =>
                    setForm((f) => ({
                      ...f,
              ...
              <div className="space-y-1">
                <Label>ブース</Label>
                <Select
                  value={(form.boothId ?? "none") as string}
                  onValueChange={(v) =>
                    setForm((f) => ({ ...f, boothId: v === "none" ? null : v }))
                  }
                >
                  <SelectTrigger>
              ...
              <div>
                ...
                <div className="flex flex-col gap-1 mt-1">
                  <DatePicker
                    value={
                      form.endDate
                        ? new Date(`${form.endDate}T00:00:00Z`)
                        : undefined
                    }
                    onChange={(d) => {
              ...
              <div>
                ...
                <div className="mt-2 flex gap-2">
                  <Button
                    ...
                    onClick={() =>
                      setForm((f) => ({ ...f, daysOfWeek: [1, 2, 3, 4, 5] }))
                    }
                  >
                    平日
                  </Button>
        ...
        >
          <DialogContent className="sm:max-w-[420px]">
            ...
            <div className="text-sm text-muted-foreground">
              このシリーズに紐づく本日以降の通常授業をキャンセル状態にします（取り消し不可）。過去の授業は変更しません。
            </div>
