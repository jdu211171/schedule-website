...
export default function ClassSeriesToolbar<TData>({ table, globalSearchPlaceholder = "検索（科目/生徒/講師）" }: Props<TData>) {
  ...
  function makeOptions(colId: string) {
    ...
    if (!col || !('getFacetedUniqueValues' in col)) return [] as Array<{ value: string; label: string }>;
  ...
  const lastGeneratedValue = (lastGeneratedCol?.getFilterValue() as (number | undefined)[] | undefined) || undefined;
  const lastGeneratedRange: DateRange | undefined = lastGeneratedValue && Array.isArray(lastGeneratedValue)
    ? {
        from: lastGeneratedValue[0] ? new Date(lastGeneratedValue[0]) : undefined,
        to: lastGeneratedValue[1] ? new Date(lastGeneratedValue[1]) : undefined,
      }
    : undefined;
    ...
    <div className="flex items-center justify-between gap-2">
      <div className="flex flex-1 items-center gap-2">
        ...
        {table.getColumn("subjectName") && subjectOptions.length > 0 && (
          <DataTableFacetedFilter column={table.getColumn("subjectName")!} title="科目" options={subjectOptions} />
        )}
        {table.getColumn("studentName") && studentOptions.length > 0 && (
          <DataTableFacetedFilter column={table.getColumn("studentName")!} title="生徒" options={studentOptions} />
        )}
        {table.getColumn("teacherName") && teacherOptions.length > 0 && (
          <DataTableFacetedFilter column={table.getColumn("teacherName")!} title="講師" options={teacherOptions} />
        )}
        {table.getColumn("dow") && (
          ...
          <SimpleDateRangePicker
            ...
            onValueChange={(range) =>
              lastGeneratedCol?.setFilterValue(
                range?.from || range?.to ? [range?.from?.getTime(), range?.to?.getTime()] : undefined
              )
            }
            placeholder="生成済み（最終）"
            showPresets
