...
import {
  ...
  TEMPLATE_EXAMPLES,
  CLASS_ITEM_VARIABLES,
  CLASS_LIST_FORMAT_EXAMPLES,
...
const formatTiming = (template: MessageTemplate) => {
  const timing =
    template.timingValue === 0 ? "当日" : `${template.timingValue}日前`;
...
export function MessageTemplateEditor({
  ...
  const generateDailyClassListPreview = (
    itemTemplate: string,
    summaryTemplate: string
  ) => {
    // Generate class list items
    const classListItems = sampleClasses
      .map((classData) => {
  ...
  const calculatePreviewValues = (
    ...
    values["dailyClassList"] = generateDailyClassListPreview(
      itemTemplate,
      summaryTemplate
    );
  ...
  useEffect(() => {
    const itemTemplate =
      localTemplate.classListItemTemplate || DEFAULT_CLASS_LIST_ITEM_TEMPLATE;
    const summaryTemplate =
      localTemplate.classListSummaryTemplate ||
      DEFAULT_CLASS_LIST_SUMMARY_TEMPLATE;
  ...
  useEffect(() => {
    ...
    localTemplate.classListItemTemplate,
    localTemplate.classListSummaryTemplate,
  ]);
  ...
  const handleUpdateTemplate = (updates: Partial<MessageTemplate>) => {
    setLocalTemplate((prev) => ({
      ...
      variables: updates.content
        ? extractTemplateVariables(updates.content)
        : prev.variables,
    }));
  };
  ...
  const insertVariable = (variableKey: string) => {
    const textarea = document.querySelector(
      'textarea[data-template-id="single"]'
    ...
    const newText =
      text.substring(0, start) + variableKey + text.substring(end);
      ...
      <Alert>
        ...
        <AlertDescription>
          <div className="space-y-2">
            <p>
              毎日、設定した時刻にその日の全ての授業をまとめた通知が送信されます。
            </p>
            <p className="text-sm">
              例: 「1日前
              09:00」に設定すると、毎日朝9時に翌日の全授業予定が通知されます。
            </p>
            <p className="text-sm font-medium">
              テンプレートを選んでメッセージをカスタマイズできます。
            </p>
          </div>
        </AlertDescription>
      </Alert>
        ...
        <CardHeader>
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              ...
              <Badge variant="outline">{formatTiming(localTemplate)}</Badge>
            </div>
            <div className="flex items-center gap-2">
              <Switch
          ...
          <Tabs defaultValue="preview" className="w-full">
            ...
            <TabsContent value="edit" className="space-y-4">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                          ...
                          <Tooltip>
                            <TooltipTrigger asChild>
                              <Badge
                                ...
                                onClick={() =>
                                  insertVariable(
                                    MESSAGE_VARIABLES.DAILY_CLASS_LIST.key
                                  )
                                }
                              >
                            ...
                            <TooltipContent>
                              ...
                              <p className="text-xs">
                                {MESSAGE_VARIABLES.DAILY_CLASS_LIST.description}
                              </p>
                            </TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
                        ...
                        {[
                          MESSAGE_VARIABLES.CLASS_COUNT,
                          MESSAGE_VARIABLES.FIRST_CLASS_TIME,
                          MESSAGE_VARIABLES.LAST_CLASS_TIME,
                          MESSAGE_VARIABLES.TOTAL_DURATION,
                        ].map((variable) => (
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {[
                          MESSAGE_VARIABLES.RECIPIENT_NAME,
                          MESSAGE_VARIABLES.RECIPIENT_TYPE,
                        ].map((variable) => (
                          <TooltipProvider key={variable.key}>
                            <Tooltip>
                              <TooltipTrigger asChild>
                    ...
                    <div>
                      ...
                      <div className="flex flex-wrap gap-2">
                        {[
                          MESSAGE_VARIABLES.CLASS_DATE,
                          MESSAGE_VARIABLES.CURRENT_DATE,
                          MESSAGE_VARIABLES.BRANCH_NAME,
                        ].map((variable) => (
                          <TooltipProvider key={variable.key}>
                            <Tooltip>
                              <TooltipTrigger asChild>
                      ...
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="w-48">
                            {Object.entries(TEMPLATE_EXAMPLES).map(
                              ([key, template]) => (
                                <DropdownMenuItem
                                  key={key}
                                  onClick={() =>
              ...
              <Textarea
                ...
                onChange={(e) =>
                  handleUpdateTemplate({ content: e.target.value })
                }
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <div className="flex items-center justify-between mb-2">
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end" className="w-48">
                            {Object.entries(CLASS_LIST_FORMAT_EXAMPLES).map(
                              ([key, format]) => (
                                <DropdownMenuItem
                                  key={key}
                                  onClick={() =>
                                    handleUpdateTemplate({
                                      classListItemTemplate:
                                        format.itemTemplate,
                                      classListSummaryTemplate:
                    ...
                    <div>
                      ...
                      <div className="mb-2">
                        ...
                        <div className="flex flex-wrap gap-1 mt-1">
                          {Object.values(CLASS_ITEM_VARIABLES).map(
                            (variable) => (
                              <TooltipProvider key={variable.key}>
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <Badge
                                      ...
                                      onClick={() => {
                                        const textarea = document.querySelector(
                                          'textarea[data-template-id="class-item"]'
                                        ) as HTMLTextAreaElement;
                                        if (!textarea) return;
                                        const start = textarea.selectionStart;
                                        const end = textarea.selectionEnd;
                      ...
                      <Textarea
                        ...
                        onChange={(e) =>
                          handleUpdateTemplate({
                            classListItemTemplate: e.target.value,
                          })
                        }
                    ...
                    <div>
                              ...
                              <Tooltip>
                                <TooltipTrigger asChild>
                                  <Badge
                                    ...
                                    onClick={() => {
                                      const textarea = document.querySelector(
                                        'textarea[data-template-id="class-summary"]'
                                      ) as HTMLTextAreaElement;
                                      if (!textarea) return;
                                      const start = textarea.selectionStart;
                                      const end = textarea.selectionEnd;
                                      ...
                                      const newText =
                                        ...
                                        text.substring(end);
                                      handleUpdateTemplate({
                                        classListSummaryTemplate: newText,
                                      });
                                      setTimeout(() => {
                                        textarea.focus();
                                        textarea.setSelectionRange(
                      ...
                      <Textarea
                        ...
                        onChange={(e) =>
                          handleUpdateTemplate({
                            classListSummaryTemplate: e.target.value,
                          })
                        }
              ...
              {(localTemplate.content.includes("{{subjectName}}") ||
                ...
                <Alert className="mb-4 border-blue-200 bg-blue-50">
                  ...
                  <AlertDescription className="text-blue-800 text-sm">
                    注:
                    このプレビューでは古い変数にサンプルデータを表示していますが、
                    実際の通知では全ての授業情報が{`{{dailyClassList}}`}
                    に統合されます。
                  </AlertDescription>
                </Alert>
              )}
