...
function computeWeeklyCadence(series: ClassSeries[] | undefined): {
  ...
  (series || []).forEach((s) =>
    (s.daysOfWeek || []).forEach((d) => set.add(d))
  );
...
export default function StudentCommutingSummary({
  studentId,
  days = 90,
}: Props) {
  const {
      ...
      <CardContent className="space-y-4">
        ...
        <div>
          <div className="text-sm text-muted-foreground">
            Total Commuting Classes / 合計（{days}日間）
          </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <div className="text-base">{loadingSeries ? "…" : expectedEnd}</div>
          </div>
        </div>
