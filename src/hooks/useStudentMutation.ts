        ...
        if (currentData?.data) {
          ...
          const optimisticStudent: Student = {
            ...
            lineNotificationsEnabled:
              newStudent.lineNotificationsEnabled ?? true,
            ...
            regularAvailability: (newStudent.regularAvailability || []).map(
              (ra) => ({
            ...
            exceptionalAvailability: (
              newStudent.exceptionalAvailability || []
            ).map((ea) => {
              ...
              return {
                date:
                  ea.date instanceof Date
                    ? ea.date.toISOString().split("T")[0]
                    : ea.date,
                timeSlots,
                fullDay: ea.fullDay,
            ...
            // Contact phones
            contactPhones: (newStudent.contactPhones || []).map(
              (phone, index) => ({
  ...
  >({
            ...
            data: currentData.data.map((student) =>
              student.studentId === updatedStudent.studentId
                ? {
                    ...
                    exceptionalAvailability:
                      updatedStudent.exceptionalAvailability
                    ...
                    absenceAvailability: (updatedStudent as any)
                      .absenceAvailability
                      ? (updatedStudent as any).absenceAvailability.map(
                          (ea: any) => {
                            const timeSlots = [] as {
                              id: string;
                              startTime: string;
    ...
    onSuccess: (data) => {
      ...
      if (updatedStudent) {
        ...
        queryClient.setQueryData(
          ["student", updatedStudent.studentId],
          updatedStudent
        );
      }
...
export function useStudentDelete() {
  const queryClient = useQueryClient();
  return useMutation<StudentsResponse, Error, string, StudentMutationContext>({
