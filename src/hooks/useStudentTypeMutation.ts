...
export function useStudentTypeOrderUpdate() {
  const queryClient = useQueryClient();
  ...
  >({
    mutationFn: (data) =>
      fetcher("/api/student-types/order", {
        method: "PATCH",
        body: JSON.stringify(data),
      }),
    onMutate: async ({ studentTypeIds }) => {
      await queryClient.cancelQueries({ queryKey: ["studentTypes"] });
      ...
      queries.forEach(([queryKey]) => {
        ...
        if (currentData) {
          const updatedData = {
            ...currentData,
            data: currentData.data.map((studentType) => {
              const newOrder = studentTypeIds.indexOf(
                studentType.studentTypeId
              );
              return newOrder !== -1
                ? { ...studentType, order: newOrder + 1 }
                : studentType;
            }),
          };
          ...
          queryClient.setQueryData<StudentTypesResponse>(queryKey, updatedData);
        }
      });

      return { previousStudentTypes };
    },
    onError: (error, _, context) => {
      if (context?.previousStudentTypes) {
        Object.entries(context.previousStudentTypes).forEach(
    ...
    onSuccess: () => {
      toast.success("生徒タイプの順序を更新しました", {
        id: "student-type-order-success",
      });
    },
    onSettled: () => {
      queryClient.invalidateQueries({
