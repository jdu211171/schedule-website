...
async function processNotifications(skipTimeCheck: boolean = false) {
  ...
  activeTemplates.forEach((template) => {
    ...
    console.log(
      `  Timing: ${template.timingValue} days before at ${timeStr} JST`
    );
  ...
  for (const template of activeTemplates) {
    try {
      ...
      const templateMinutes =
        template.timingHour * 60 + (template.timingMinute ?? 0);
      ...
      if (!skipTimeCheck && !shouldSend) {
        ...
        console.log(
          `⏰ Waiting for scheduled time: ${scheduledTime}. Current time: ${currentTime}`
        );
        continue;
      }
      ...
      console.log(
        `    Today start (JST): ${format(todayStartJST, "yyyy-MM-dd HH:mm:ss")}`
      );
      console.log(
        `    Today start (UTC): ${format(todayStartUTC, "yyyy-MM-dd HH:mm:ss")}`
      );
      ...
      const alreadyProcessed = await prisma.notification.count({
        where: {
          ...
          notificationType: {
            contains:
              template.timingValue === 0
                ? "SAMEDAY"
                : template.timingValue === 1
                  ? "24H"
      ...
      if (!skipTimeCheck && alreadyProcessed > 0) {
        console.log(
          `✅ Already processed today for ${template.name} (target: ${targetDateString})`
        );
        continue;
      }
      ...
      console.log(
        `  Target date object (JST): ${format(targetDate, "yyyy-MM-dd HH:mm:ss")}`
      );
      console.log(`  Target date object (UTC): ${targetDate.toISOString()}`);
      console.log(`  Template ID: ${template.id}`);
      console.log(`  Branch: ${template.branchId || "Global (all branches)"}`);
      console.log(
        `  Searching for classes on date: ${format(targetDate, "yyyy-MM-dd")}`
      );
      console.log(
        `  Date range: ${targetDate.toISOString()} to ${new Date(targetDate.getTime() + 24 * 60 * 60 * 1000).toISOString()}`
      );
      ...
      console.log(
        `  Found ${teachersWithClasses.length} teachers with classes on ${targetDateString}`
      );
      ...
      console.log(
        `  Found ${studentsWithDirectClasses.length} students with direct classes on ${targetDateString}`
      );

      // Removed studentsWithEnrolledClasses query as we no longer support group classes

      const recipientSessions = new Map<
        string,
        {
      ...
      const totalSessions =
        recipientSessions.size > 0
          ? Array.from(recipientSessions.values()).reduce(
              (sum, recipient) => sum + recipient.sessions.length,
              0
            )
          : 0;
      console.log(
      ...
      if (recipientSessions.size === 0) {
        console.log(
          "  ⚠️ No recipients with sessions found - skipping this template"
        );
        continue;
      }
        ...
        try {
          ...
          for (const [branchId, branchSessions] of sessionsByBranch) {
            ...
            if (await isVacationDay(effectiveBranchId, targetDate)) {
              console.log(
                `  ⛔ Skipping notifications for branch ${effectiveBranchId} on vacation day ${targetDateString}`
              );
              continue;
            }

            const itemTemplate =
              template.classListItemTemplate ||
              DEFAULT_CLASS_LIST_ITEM_TEMPLATE;
            const summaryTemplate =
              template.classListSummaryTemplate ||
              DEFAULT_CLASS_LIST_SUMMARY_TEMPLATE;
            let dailyClassList = "";
            branchSessions.forEach((session, index) => {
              ...
              dailyClassList +=
                replaceTemplateVariables(
                  recipientSpecificTemplate,
                  classItemVariables
                ) + (index < branchSessions.length - 1 ? "\n\n" : "");
            });
            ...
            if (summaryTemplate && branchSessions.length > 0) {
              ...
              const summaryVariables = {
                ...
                firstClassTime: firstSession
                  ? `${String(firstStartDate.getUTCHours()).padStart(2, "0")}:${String(firstStartDate.getUTCMinutes()).padStart(2, "0")}`
                  : "",
                lastClassTime: lastSession
                  ? `${String(lastEndDate.getUTCHours()).padStart(2, "0")}:${String(lastEndDate.getUTCMinutes()).padStart(2, "0")}`
                  : "",
              };
              dailyClassList +=
                "\n\n" +
            ...
            if (branchSessions.length > 0) {
              ...
              const sortedSessions = [...branchSessions].sort(
                (a, b) =>
            ...
            const templateVariables = {
              ...
              recipientType:
                recipient.recipientType === "TEACHER" ? "講師" : "生徒",
            ...
            const notificationType =
              template.timingValue === 0
                ? "DAILY_SUMMARY_SAMEDAY"
                : `DAILY_SUMMARY_${template.timingValue}D`;

            const scheduledAtJST = startOfDay(nowJST);
            scheduledAtJST.setHours(
              template.timingHour ?? 9,
              template.timingMinute ?? 0,
              0,
              0
            );

            const scheduledAt = skipTimeCheck
              ? now
              : fromZonedTime(scheduledAtJST, TIMEZONE);
            ...
            console.log(
              `  Branch: ${branchName || "Global"} (ID: ${notificationBranchId || "none"})`
            );
            console.log(`  Classes in branch: ${branchSessions.length}`);
            console.log(
              `  Scheduled at (UTC): ${format(scheduledAt, "yyyy-MM-dd HH:mm:ss")}`
            );
            ...
            const notification = await createNotification({
              ...
              relatedClassId:
                branchSessions.length > 0
                  ? branchSessions.map((s) => s.classId).join(",")
                  : undefined,
            ...
            if (notification) {
              console.log(
                `  ✅ QUEUED: Notification ${notification.notificationId} queued for ${recipient.name}`
              );
  ...
  console.log(
    "ℹ️ Notifications will be sent by the worker when scheduled time arrives"
  );
...
export async function GET(req: NextRequest) {
  try {
    console.log(
      "📬 Notification SEND cron triggered at:",
...
export const POST = withRole(["ADMIN"], async (req: NextRequest) => {
  try {
    ...
    console.log(
      "Manual notification trigger requested by admin with skipTimeCheck:",
      skipTimeCheck
    );
