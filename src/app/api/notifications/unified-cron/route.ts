  ...
  try {
    ...
    for (const template of activeTemplates) {
      try {
        ...
        console.log(
          `  Scheduled for: ${template.timingHour}:${String(template.timingMinute ?? 0).padStart(2, "0")}`
        );
        ...
        if (!reset && (minutesDiff < 0 || minutesDiff >= 15)) {
          console.log(
            `  ⏰ Not in time window. Current: ${currentHour}:${String(currentMinute).padStart(2, "0")}, Scheduled: ${templateHour}:${String(templateMinute).padStart(2, "0")}`
          );
        ...
        console.log(
          `  ✅ Within time window (${minutesDiff} minutes after scheduled time)`
        );
        ...
        if (reset) {
          ...
          if (deleted.count > 0) {
            console.log(
              `  🗑️ Reset: Deleted ${deleted.count} PENDING notifications for template ${template.name}`
            );
        ...
        if (!reset) {
          ...
          if (alreadyProcessed > 0) {
            console.log(
              `  ✅ Already processed today for target date ${targetDateString}`
            );
            continue;
          }
        }
        ...
        console.log(
          `  Found ${teachersWithClasses.length} teachers with classes`
        );
        ...
        console.log(
          `  Found ${studentsWithClasses.length} students with classes`
        );
          ...
          try {
            ...
            for (const [branchId, sessions] of sessionsByBranch) {
              ...
              if (await isVacationDay(branchId, targetDate)) {
                console.log(
                  `  ⛔ Skipping notifications for branch ${branchId} on vacation day ${format(targetDate, "yyyy-MM-dd")}`
                );
                continue;
              }
              const itemTemplate =
              ...
              sessions.forEach((session: any, index: number) => {
                ...
                if (!session.student?.name || !session.teacher?.name) {
                  console.warn(
                    `⚠️ Class session ${session.classId} is missing ${!session.student?.name ? "student" : "teacher"} information`
                  );
                }
              ...
              const totalDuration =
                hours > 0
                  ...
                  : `${minutes}分`;
              const branchName =
                sortedSessions[0].branch?.name || template.branch?.name || "";
              ...
              console.log(
                `    ✅ Created notification for ${recipient.recipientType} ${recipient.name} (branch ${branchName})`
              );
            }
          } catch (error) {
            console.error(
              `    ❌ Failed to create notification for ${recipient.name}:`,
              error
            );
            results.errors.push(
              `Failed to create notification for ${recipient.name}: ${error instanceof Error ? error.message : String(error)}`
            );
          }
        }
      } catch (error) {
        ...
        results.errors.push(
          `Template ${template.name}: ${error instanceof Error ? error.message : String(error)}`
        );
      }
    }
    ...
    console.log(
      `✅ Worker completed: Sent ${workerResult.successful}, Failed ${workerResult.failed}`
    );
    ...
    console.log(
      `Total notifications deleted (reset): ${results.deletedNotifications}`
    );
