...
export const GET = withBranchAccess(
  ["ADMIN", "STAFF", "TEACHER"],
  async (request: NextRequest, _session, selectedBranchId) => {
    const { searchParams } = new URL(request.url);
    const studentId = searchParams.get("studentId");
    const teacherId = searchParams.get("teacherId");
    const daysParam =
      searchParams.get("days") ?? searchParams.get("windowDays");
    ...
    const uniqueTypeIds = Array.from(
      new Set(sessions.map((s) => s.classTypeId).filter(Boolean))
    ) as string[];
    ...
    const teacherIds = Array.from(
      new Set(seriesRows.map((s) => s.teacherId).filter(Boolean))
    ) as string[];
    const studentIds = Array.from(
      new Set(seriesRows.map((s) => s.studentId).filter(Boolean))
    ) as string[];
    const subjectIds = Array.from(
      new Set(seriesRows.map((s) => s.subjectId).filter(Boolean))
    ) as string[];
      ...
      seriesRows.map(async (s) => {
        ...
        return {
          ...
          teacherName: s.teacherId
            ? (teacherMap.get(s.teacherId) ?? null)
            : null,
          studentId: s.studentId,
          studentName: s.studentId
            ? (studentMap.get(s.studentId) ?? null)
            : null,
          subjectId: s.subjectId,
          subjectName: s.subjectId
            ? (subjectMap.get(s.subjectId) ?? null)
            : null,
