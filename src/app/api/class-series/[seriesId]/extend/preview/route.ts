  ...
  async (request: NextRequest, session, selectedBranchId) => {
    try {
      ...
      const months = Math.max(
        1,
        Math.min(6, Number(url.searchParams.get("months") ?? 1))
      );
      ...
      const startBaseline = series.lastGeneratedThrough
        ? new Date(series.lastGeneratedThrough)
        : new Date(series.startDate);
      startBaseline.setUTCHours(0, 0, 0, 0);
      let fromDate = series.lastGeneratedThrough
        ? new Date(startBaseline.getTime() + 86400000)
        : startBaseline;
      const minBound = new Date(series.startDate);
      minBound.setUTCHours(0, 0, 0, 0);
      if (fromDate < minBound) fromDate = minBound;
      if (fromDate < today) fromDate = today;
      let toDate = addMonths(fromDate, months);
      toDate.setUTCHours(23, 59, 59, 999);
      if (series.endDate) {
        const hardEnd = new Date(series.endDate);
      ...
      const dows: number[] = Array.isArray(dowsRaw)
        ? dowsRaw.map(Number).filter(Number.isFinite)
        : [];
      ...
      if (candidates.length === 0) {
        return NextResponse.json({
          conflicts: [],
          conflictsByDate: {},
          message: "No matching days in range",
          requiresConfirmation: false,
          summary: {
            totalSessions: 0,
            sessionsWithConflicts: 0,
      ...
      const [teacher, student] = await Promise.all([
        series.teacherId
          ? prisma.teacher.findUnique({
              where: { teacherId: series.teacherId },
              select: { userId: true, name: true },
            })
          : Promise.resolve(null),
        series.studentId
          ? prisma.student.findUnique({
              where: { studentId: series.studentId },
              select: { userId: true, name: true },
            })
          : Promise.resolve(null),
      ]);
      ...
      async function slotsForUser(
        ...
        for (const r of recs) {
          if (r.fullDay) slots.push({ startTime: "00:00", endTime: "23:59" });
            ...
            slots.push({
              startTime: minutesToHHmm(toMin(r.startTime)),
              endTime: minutesToHHmm(toMin(r.endTime)),
            });
          }
        }
        return slots;
      ...
      for (const date of candidates) {
        ...
        for (const s of sessions) {
          ...
          if (
            series.teacherId &&
            s.teacherId === series.teacherId &&
            !added.has("TEACHER_CONFLICT")
          ) {
            conflicts.push({
              date: dateStr,
              dayOfWeek: day,
              ...
              participant: teacher
                ? { id: series.teacherId!, name: teacher.name, role: "teacher" }
                : undefined,
              sharedAvailableSlots: [],
              teacherSlots: [],
              studentSlots: [],
            });
          ...
          if (
            series.studentId &&
            s.studentId === series.studentId &&
            !added.has("STUDENT_CONFLICT")
          ) {
            conflicts.push({
              date: dateStr,
              dayOfWeek: day,
              ...
              participant: student
                ? { id: series.studentId!, name: student.name, role: "student" }
                : undefined,
              sharedAvailableSlots: [],
              teacherSlots: [],
              studentSlots: [],
          ...
          if (
            series.boothId &&
            s.boothId === series.boothId &&
            !added.has("BOOTH_CONFLICT")
          ) {
            conflicts.push({
              date: dateStr,
              dayOfWeek: day,
        ...
        const reqE = eh * 60 + em;
        const s2m = (s: string) =>
          parseInt(s.slice(0, 2)) * 60 + parseInt(s.slice(3, 5));
        const inTeacher = tSlots.some(
          (sl) => reqS >= s2m(sl.startTime) && reqE <= s2m(sl.endTime)
        );
        const inStudent = sSlots.some(
          (sl) => reqS >= s2m(sl.startTime) && reqE <= s2m(sl.endTime)
        );
        if (teacher && !inTeacher) {
          const reason =
            tSlots.length === 0 ? "TEACHER_UNAVAILABLE" : "TEACHER_WRONG_TIME";
          if (!allowOutside.teacher) {
            conflicts.push({
              date: dateStr,
              dayOfWeek: day,
              type: reason,
              details: `${teacher.name}先生は${reason === "TEACHER_UNAVAILABLE" ? "この日に利用可能時間が設定されていません" : "指定された時間帯に利用できません。"}`,
              participant: {
                id: series.teacherId!,
                name: teacher.name,
                role: "teacher",
        ...
        if (teacher && student) {
          const tCovers = tSlots.some(
            (sl) => reqS >= s2m(sl.startTime) && reqE <= s2m(sl.endTime)
          );
          const sCovers = sSlots.some(
            (sl) => reqS >= s2m(sl.startTime) && reqE <= s2m(sl.endTime)
          );
          let bothCoverWindow = false;
          if (tCovers && sCovers) {
            bothCoverWindow = tSlots.some((ta) =>
              sSlots.some(
                (sb) =>
                  Math.max(s2m(ta.startTime), s2m(sb.startTime)) <= reqS &&
                  Math.min(s2m(ta.endTime), s2m(sb.endTime)) >= reqE
              )
            );
          }
        ...
        if (student && !inStudent) {
          const reason =
            sSlots.length === 0 ? "STUDENT_UNAVAILABLE" : "STUDENT_WRONG_TIME";
          if (!allowOutside.student) {
            conflicts.push({
              date: dateStr,
              dayOfWeek: day,
              type: reason,
              details: `${student.name}さんは${reason === "STUDENT_UNAVAILABLE" ? "この日に利用可能時間が設定されていません" : "指定された時間帯に利用できません。"}`,
              participant: {
                id: series.studentId!,
                name: student.name,
                role: "student",
      ...
      const conflictsByDate: Record<string, ConflictInfo[]> = conflicts.reduce(
        (acc, c) => {
          (acc[c.date] ||= []).push(c);
          return acc;
        },
      ...
      return NextResponse.json({
        ...
        conflictsByDate,
        message:
          summary.sessionsWithConflicts > 0
            ? `繰り返しクラスの作成中に${summary.sessionsWithConflicts}日分で競合が見つかりました。`
            : "競合は見つかりませんでした。",
      ...
      return NextResponse.json(
        { error: "Failed to preview series extension" },
        { status: 500 }
      );
    }
  }
