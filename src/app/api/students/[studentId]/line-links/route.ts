...
export const GET = withBranchAccess(['ADMIN', 'STAFF'], async (req: NextRequest, session, branchId) => {
...
export const POST = withBranchAccess(['ADMIN', 'STAFF'], async (req: NextRequest, session, branchId) => {
  ...
  if (existingLink || existingTeacherLink) {
    return NextResponse.json(
      { error: 'This LINE ID is already linked to another account on this channel' },
      { status: 409 }
    );
  }
  ...
  if (accountSlot === 'parent') {
    ...
    if (existingParent && existingParent.lineUserId !== lineUserId) {
      return NextResponse.json(
        { error: 'A different parent LINE account is already linked for this student. Please unlink it first.' },
        { status: 409 }
      );
    }
  }
...
export const DELETE = withBranchAccess(['ADMIN', 'STAFF'], async (req: NextRequest, session, branchId) => {
  const urlParts = req.url.split('/');
  const studentId = urlParts[urlParts.length - 2]; // Get studentId from URL
  const userBranches = session.user?.branches || [];
  ...
  try {
    if (linkId) {
      ...
      return NextResponse.json(
        { error: 'Either linkId or both channelId and accountSlot must be provided' },
        { status: 400 }
      );
    }
