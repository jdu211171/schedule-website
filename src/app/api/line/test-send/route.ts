      ...
      try {
        ...
        if (notification) {
          try {
            ...
            const credentials = await import("@/lib/line-multi-channel").then(
              (m) => m.getChannelCredentials(branchId || undefined)
            );
            if (credentials) {
              await sendLineMulticast([lineId], message, credentials);
            } else {
              // Fallback to basic LINE function if no credentials found
              const { sendLineMulticast: fallbackMulticast } = await import(
                "@/lib/line"
              );
