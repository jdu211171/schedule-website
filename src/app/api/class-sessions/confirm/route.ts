...
export const POST = withBranchAccess(
  ...
  async (request: NextRequest, session, selectedBranchId) => {
    ...
    const results: {
      updated: string[];
      failed: { classId: string; reason: string }[];
    } = { updated: [], failed: [] };
    ...
    for (const classId of body.classIds) {
      try {
        ...
        if (
          target.branchId &&
          selectedBranchId &&
          target.branchId !== selectedBranchId &&
        ...
        if (target.teacherId || target.studentId || target.boothId) {
          ...
          for (const s of sameDay) {
            ...
            if (target.teacherId && s.teacherId === target.teacherId)
              reasons.push({ type: "TEACHER_CONFLICT" });
            if (target.studentId && s.studentId === target.studentId)
              reasons.push({ type: "STUDENT_CONFLICT" });
            if (target.boothId && s.boothId === target.boothId)
