      ...
      try {
        ...
        if (!availability.available) {
          ...
          if (!availability.user1.available) {
            type =
              availability.user1.conflictType === "UNAVAILABLE"
                ? "TEACHER_UNAVAILABLE"
                : "TEACHER_WRONG_TIME";
            details =
              type === "TEACHER_UNAVAILABLE"
                ? `${target.teacher?.name ?? "講師"}はこの日に利用可能時間が設定されていません`
                : `${target.teacher?.name ?? "講師"}は指定された時間帯に利用できません。`;
            participant = {
              id: target.teacherId!,
            ...
            type =
              availability.user2.conflictType === "UNAVAILABLE"
                ? "STUDENT_UNAVAILABLE"
                : "STUDENT_WRONG_TIME";
            details =
              type === "STUDENT_UNAVAILABLE"
                ? `${target.student?.name ?? "生徒"}はこの日に利用可能時間が設定されていません`
                : `${target.student?.name ?? "生徒"}は指定された時間帯に利用できません。`;
            participant = {
              id: target.studentId!,
