...
export const classTypeColorClasses: Record<
  ClassTypeColor,
  {
    background: string;
    border: string;
...
export function getHexForClassTypeColor(
...
export function isValidClassTypeColor(
  value: string | null | undefined
): value is ClassTypeColor {
  return (
    !!value && (CLASS_TYPE_DEFAULT_COLORS as readonly string[]).includes(value)
  );
}
