...
export function handleImportError(error: unknown) {
  ...
  if (error instanceof Error) {
    if (error.name === "PrismaClientValidationError") {
      ...
      const invalidValueMatch = error.message.match(/Invalid value for argument `(\w+)`/);
      ...
      return NextResponse.json(
        {
          error: "データ形式が不正です。CSVファイルが必要なテンプレート形式と一致しているか確認してください。"
        },
        { status: 400 }
      );
    }
    ...
    if (error.message.includes("Unique constraint")) {
      ...
      return NextResponse.json(
        {
          error: "重複データが見つかりました。一部のレコードは既にデータベースに存在している可能性があります。"
        },
        { status: 400 }
      );
    }
    ...
    if (error.message.includes("Foreign key constraint")) {
      return NextResponse.json(
        {
          error: "無効な参照が見つかりました。参照されているデータ（校舎、科目など）がシステムに存在することを確認してください。"
        },
        { status: 400 }
      );
    }
  ...
  return NextResponse.json(
    {
      error: "インポート中に予期しないエラーが発生しました。データを確認して、もう一度お試しください。"
    },
    { status: 500 }
  );
}